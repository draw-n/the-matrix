...
import { getAllAnnouncements, editAnnouncementById, createAnnouncement, deleteAnnouncementById } from "../api/announcement";
...
export const useCreateAnnouncement = () => {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: (newAnnouncement: Partial<Announcement>) => createAnnouncement(newAnnouncement),
        onSuccess: () => {
...
export const useDeleteAnnouncementById = () => {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({announcementId}: {announcementId: string}) => deleteAnnouncementById(announcementId),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["announcements"] });

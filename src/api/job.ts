import axios from "axios";
import { Job } from "../types/job";

export const getAllJobs = async (
    statuses?: string[],
    equipmentId?: string,
    userId?: string,
) => {
    try {
        let query = "";
        if (equipmentId && userId && statuses && statuses.length > 0) {
            query += `equipmentId=${equipmentId}&userId=${userId}&status=${statuses.join(",")}`;
        } else if (userId && statuses && statuses.length > 0) {
            query += `userId=${userId}&status=${statuses.join(",")}`;
        } else if (equipmentId && statuses && statuses.length > 0) {
            query += `equipmentId=${equipmentId}&status=${statuses.join(",")}`;
        } else if (userId && equipmentId) {
            query += `userId=${userId}&equipmentId=${equipmentId}`;
        } else if (equipmentId) {
            query += `equipmentId=${equipmentId}`;
        } else if (userId) {
            query += `userId=${userId}`;
        } else if (statuses && statuses.length > 0) {
            query += `status=${statuses.join(",")}`;
        }
        const response = await axios.get<Job[]>(
            `${import.meta.env.VITE_BACKEND_URL}/jobs?${query}`,
        );
        return response.data;
    } catch (error) {
        console.error("Error fetching jobs:", error);
    }
};

export const getJobChartData = async (userId?: string, days?: number) => {
    try {
        const response = await axios.get(
            `${import.meta.env.VITE_BACKEND_URL}/jobs/chart-data?userId=${userId}&days=${days}`,
        );
        return response.data;
    } catch (error) {
        console.error("Error fetching job chart data:", error);
    }
};

export const getFilamentUsedGrams = async (userId?: string) => {
    try {
        const response = await axios.get(
            `${import.meta.env.VITE_BACKEND_URL}/jobs/filament-usage?userId=${userId}`,
        );
        return response.data.totalFilamentUsed;
    } catch (error) {
        console.error("Error fetching filament usage data:", error);
    }
};
